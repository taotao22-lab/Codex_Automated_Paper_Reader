...
- 支持每天 7:00 自动检索 3 天前的文章，生成日报、提交并推送到 GitHub。
- 自动提交信息要求使用中文。
...
这一步只生成候选池，不生成最终日报。默认示例使用 `3-days-ago`，也就是检索运行当天往前 3 天的文章，避免当天源站批次尚未更新。`data/raw/YYYY-MM-DD.json` 会包含 `duplicate_check` 和 `recommended_action`。如果状态提示没有新批次或候选池重复，Codex 应写一段“无新候选批次”说明，而不是重复生成旧 Top 10。
...
- 使用中文提交信息，例如：`文档：新增 YYYY-MM-DD Agent 文章日报`，其中 `YYYY-MM-DD` 是被检索的 3 天前日期。
- 推送到 GitHub。
